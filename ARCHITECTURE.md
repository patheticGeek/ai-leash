...
graph TD
    ...
    chat --> tools[tools.rs\ntool schemas + execution + permissions]
...
| `tools.rs`           | Every built-in tool's JSON schema + execution (`read_file`, `edit_file`, `write_file`, `list_dir`, `grep`, `shell`, `update_memory`, `load_skill`, `spawn_sub_agent`, `list_sub_agents`, `read_sub_agent`, plus the Actions tool arms) and the permission-request flow (`request_permission`, blocking on a oneshot channel until the UI responds). |
