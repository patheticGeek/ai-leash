...
graph TD
    ...
    acp[acp/\nregistry: subprocess lifecycle + event mapping\n+ permission bridging + discovery] --> state
    acp --> chat
    acp --> mcpsrv
...
| `chat/`              | Built-in agent module: `mod.rs` holds the shared `ChatMessage`/`TurnResult` types and the top-level `#[tauri::command]`s (`send_prompt`, `retry_last`, `cancel_prompt`, `clear_conversation`, `compact_conversation`); `agent_loop.rs` holds `run_agent_loop` (capped at 15 tool iterations, streaming orchestration, system-prompt refresh every turn) and `run_with_cancellation` (per-session serialization + the `generating` event), plus the shared persistence helpers (`push_message`, `remember_in_memory`, `start_streaming_assistant_message`); `sub_agents.rs` holds the always-async `run_sub_agent` / `resume_after_background_subtask` machinery; `history.rs` holds `load_conversation_history`, `list_sub_agents`, `delete_sub_agent`. Adding a new top-level chat command is a new arm in `mod.rs` delegating into the matching file, not a growing single file. |
| `provider/`           | Provider registry: `mod.rs` holds `ProviderConfig` (`Ollama` / `OpenAiCompatible`), `ProviderError::{Transient,Fatal}` (drives retry logic), `ModelSummary`, and the public dispatchers (`stream_turn`, `complete`, `list_provider_models`, `check_provider_connection`, all `#[tauri::command]`s where applicable) — each just matches the enum and delegates one line per variant, plus the streaming helpers shared across providers (`persist_chunk`, `LineEffects`/`emit_line_effects`); `ollama.rs` holds Ollama's NDJSON streaming/completion/model-listing; `openai_compat.rs` holds the OpenAI-compatible SSE streaming/completion/connectivity-check. Adding a third provider kind is a new file shaped like `ollama.rs` plus one enum variant and one match arm per dispatcher in `mod.rs`, not a third branch threaded through every function. |
...
| `db/`                | SQLite persistence split by table: `mod.rs` holds the `Db` handle itself, connection opening, WAL-mode pragma, schema creation, and the shared `now()` timestamp helper; `conversations.rs` holds the `conversations`-row upsert shared by every message write plus `clear_conversation` (a whole-conversation delete that cascades into `messages` and `sub_agents`, so it lives here rather than being split further); `messages.rs` holds `PersistedMessage` and the `messages` table's save/stream/load functions; `sub_agents.rs` holds `SubAgentSummary`/`SubAgentMeta` and the `sub_agents` table's start/finish/list/delete functions. No `#[tauri::command]`s live here — everything is called from `chat/`, `tools/`, `acp/`. Adding a new persisted table (e.g. Actions history) is a new file here, not more lines in an existing one.                                                                                                                                                                                                                                        |
| `acp/`               | Drives an external ACP agent subprocess as an alternative to `chat.rs`'s loop: `mod.rs` holds the public `#[tauri::command]` entry points (`send_prompt_acp`, `set_acp_model`, `fetch_acp_models`) and re-exports `AcpCommand`; `process.rs` holds subprocess lifecycle (`ensure_acp_session`, `run_acp_session`, `drive_acp_connection`) and the `AcpCommand` enum sent into a running session's channel; `events.rs` maps ACP session notifications onto the same `chat://` events the built-in loop emits, and persists tool calls/results; `permissions.rs` bridges an ACP `RequestPermissionRequest` onto AI Leash's own approve/deny UI; `discovery.rs` builds the MCP server list handed to a new session, and the model/command-list payloads sent to the frontend. Adding support for a new external ACP agent's quirks is mostly a `discovery.rs` change, not an edit to the subprocess driver. |
...
graph TD
    ...
    SettingsModal --> AgentsSettingsTab
...
| `store/`                                                                         | The zustand store, split into per-concern slices combined in `index.ts` (exports `useAppStore`/`AppStore`, unchanged for callers): `projectSlice` (project root, open files, recent projects), `providerSlice` (a *list* of Ollama connections + OpenAI-compatible configs, per-config connectivity/model cache, settings-modal open state), `acpSlice` (ACP agent config, model cache, per-conversation backend choice), `backendSlice` (`defaultBackend` — the one shared "which agent answers a brand-new conversation" setting, replacing the old `providerSettings.activeId`/`agentBackend.kind`/`activeAcpId` trio), `permissionSlice` (permission mode, pending-permission map), `subAgentSlice` (sub-agent tasks/threads), `panelSlice` (panel/chat tabs per-project, generating-session state). Slice actions freely read/call other slices via `get()`, typed against the full `AppStore`. Adding a new feature's state is a new slice file plus one line in `index.ts`, not more lines in one growing file. |
...
| `settings/SettingsModal.tsx`                                                     | Tab shell only: modal chrome, the side-nav tab strip, active-section state; renders one of the `tabs/*.tsx` below. Not registry-driven like `tabKinds.ts` — three fixed tabs, no runtime extension point needed yet.                                                                  |
| `settings/tabs/AgentsSettingsTab.tsx`                                            | The merged "Providers" + "Agents" settings page: every configured Ollama connection, OpenAI-compatible API, and ACP agent renders as one flat grid of cards (`TabPicker.tsx`'s tile visual language), each with an edit/delete affordance and a click-to-make-default toggle backed by `backendSlice.ts`'s `defaultBackend`; a trailing "Add" tile opens a type picker then that type's form. Deliberately never surfaces the builtin-vs-ACP distinction — a card is just "an agent," matching `useChatSession.ts`'s chat-bar picker framing.                                     |
